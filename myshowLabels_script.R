...
##################
# 2022-09-11 Modified printing: only the outlier cities Nairobi and Karachi are printed (hard coded line 138)
...
myshowLabels <- function(x, y, labels=NULL, method="identify",
  ...
  for (meth in method){
    ...
    res <- c(res, myshowLabels1(x, y, labels, meth, n, cex,
                              col, location, ...))
  }
  return(if(is.null(res)) invisible(res) else res)
}

myshowLabels1 <- function(x, y, labels=NULL, id.method="identify",
  ...
  if (id.location != "avoid"){
    ...
    # print only the outlier cities Nairobi and Karachi
    for (i in ind) {
      if (labels[i] %in% c("Karachi", "Nairobi") ) {
        text(x[i], y[i], labels[i], cex = id.cex, xpd = TRUE,
             col = id.col, pos = labpos[i], offset = 0.25)
        info(logger = default_logger, paste("ollaanko jossain",labels[i] ))
